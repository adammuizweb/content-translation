...
$check(($manifest['version'] ?? '') === '1.16.0' && ($manifest['requires']['jyavani'] ?? '') === '>=2.3.116' && str_contains((string)($manifest['assets']['css'][0] ?? ''), 'v=1.16.0'), 'manifest keeps version/cache key aligned and requires the focused Core media workflow contract');
