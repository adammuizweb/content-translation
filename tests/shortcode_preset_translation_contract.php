...
$check(($manifest['version'] ?? '') === '1.19.1' && ($manifest['requires']['jyavani'] ?? '') === '>=2.3.155' && str_contains((string)($manifest['assets']['css'][0] ?? ''), 'v=1.19.1'), 'manifest keeps version/cache key aligned and requires the Core list and editor contracts');
