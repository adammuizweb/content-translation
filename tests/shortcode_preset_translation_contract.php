...
$check(str_contains($source['helpers'], "'version' => 5") && str_contains($source['helpers'], "'shortcode_preset_translations'"), 'versioned export includes Shortcode Preset translations');
...
$check(($manifest['version'] ?? '') === '1.13.1' && ($manifest['requires']['jyavani'] ?? '') === '>=2.3.96' && str_contains((string)($manifest['assets']['css'][0] ?? ''), 'v=1.13.1'), 'manifest keeps version/cache key and requires workflow and multilingual theme Core contracts');
