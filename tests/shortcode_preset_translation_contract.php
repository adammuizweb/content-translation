...
$check(($manifest['version'] ?? '') === '1.14.1' && ($manifest['requires']['jyavani'] ?? '') === '>=2.3.108' && str_contains((string)($manifest['assets']['css'][0] ?? ''), 'v=1.14.1'), 'manifest keeps version/cache key aligned and requires the Core media extension contract');
