...
$check(($manifest['version'] ?? '') === '1.15.0' && ($manifest['requires']['jyavani'] ?? '') === '>=2.3.114' && str_contains((string)($manifest['assets']['css'][0] ?? ''), 'v=1.15.0'), 'manifest keeps version/cache key aligned and requires the locale-aware Core collection contract');
