...
$check(($manifest['version'] ?? '') === '1.15.1' && ($manifest['requires']['jyavani'] ?? '') === '>=2.3.115' && str_contains((string)($manifest['assets']['css'][0] ?? ''), 'v=1.15.1'), 'manifest keeps version/cache key aligned and requires the Core collection settings extension contract');
