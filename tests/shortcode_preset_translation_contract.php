...
$check(str_contains($source['list'], "current_user_role(\$pdo) !== 'admin'") && str_contains($source['editor'], "current_user_role(\$pdo) !== 'admin'"), 'preset list and editor enforce admin role in depth');
$check(str_contains($source['save'], "REQUEST_METHOD") && str_contains($source['save'], "!== 'POST'"), 'preset mutation endpoint requires POST');
$check(str_contains($source['save'], "current_user_role(\$pdo) !== 'admin'") && str_contains($source['save'], 'csrf_check'), 'preset mutation endpoint enforces admin and CSRF in depth');
...
$check(str_contains($source['bootstrap'], "'shortcode_preset_translations'") && str_contains($source['export'], "current_user_role(\$pdo) !== 'admin'"), 'uninstall drops preset translations and export enforces admin role');
...
$check(($manifest['version'] ?? '') === '1.11.0' && ($manifest['requires']['jyavani'] ?? '') === '>=2.3.58' && str_contains((string)($manifest['assets']['css'][0] ?? ''), 'v=1.11.0'), 'manifest, Core requirement, and CSS cache key are 1.11.0');
