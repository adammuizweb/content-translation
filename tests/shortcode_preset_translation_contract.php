...
$check(($manifest['version'] ?? '') === '1.17.0' && ($manifest['requires']['jyavani'] ?? '') === '>=2.3.140' && str_contains((string)($manifest['assets']['css'][0] ?? ''), 'v=1.17.0'), 'manifest keeps version/cache key aligned and requires the scoped Core editor contract');
