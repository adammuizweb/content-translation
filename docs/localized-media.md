...
An alternate media translation can be marked for deletion in the media detail panel. The deletion is checked against its optimistic token and is applied only when the CSRF-protected Core metadata form is saved; Core metadata is never deleted.
